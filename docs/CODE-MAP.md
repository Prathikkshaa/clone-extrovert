...
- `app/core/inbox.service.ts` + `app/core/me.service.ts` — inbox client (conversations, thread, draft/send reply) + profile client (`GET`/`PUT /me`). `app/pages/inbox/*` — protected **threaded inbox**: conversations w/ reply-label chip, full thread in order, AI **Draft reply** + editable composer (nothing sends until Send). Settings gains the **mailing-address** field (compliance). (File 11)
