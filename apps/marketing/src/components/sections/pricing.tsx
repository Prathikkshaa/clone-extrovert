...
// Best per-credit pack (a computed fact, used as an honest highlight).
...
export function Pricing({ withHeading = true }: { withHeading?: boolean }) {
  return (
    <section className="shell py-section-y">
      ...
      <Reveal delay={0.1} className="mt-8">
        <div className="rounded-xl border border-line bg-surface p-6 md:p-8">
          <p className="text-heading-sm text-ink">
            One simple unit: credits. 1 credit ≈ {usd(CREDIT_USD_CENTS)}.
          </p>
      ...
      {/* Top-up packs — real values from shared. */}
            ...
            <Reveal key={pack.id} delay={0.05 * i}>
              ...
              >
                ...
                <p className="mt-1 text-body-sm text-muted">
                  {pack.credits.toLocaleString('en-US')} credits · {perCredit.toFixed(1)}¢ each
                </p>
                <div className="mt-6">
                  <CtaButton
                    href={SIGNUP_URL}
