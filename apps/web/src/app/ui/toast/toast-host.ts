  ...
  template: `
    ...
    >
      @for (t of toasts(); track t.id) {
        <div
          class="ui-toast pointer-events-auto flex w-full max-w-sm items-start gap-3 rounded-lg border border-line bg-surface p-3 shadow-lg"
          [attr.role]="t.kind === 'error' ? 'alert' : 'status'"
        >
          <span class="mt-0.5 shrink-0" [class]="toneText(t.kind)">
            <ui-icon [name]="icon(t.kind)" [size]="18" />
          </span>
          <p class="flex-1 text-sm text-ink">{{ t.message }}</p>
          <button
            type="button"
            class="shrink-0 rounded text-muted transition-colors duration-200 hover:text-ink focus:outline-none focus-visible:ring-2 focus-visible:ring-accent"
            (click)="dismiss(t.id)"
